...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		*/
		...
		var tipText:FlxText = new FlxText(UI_box.x, UI_box.y + UI_box.height + 6, 0,
			...
			\nZ/X - Zoom in/out
			\nComma/Period - Zoom grid out/in
			\nEnter - Test your chart
			\nQ/E - Decrease/Increase Note Sustain Length
			\nSpace - Stop/Resume song
			\nR - Reset section
			\nF1 - Hide/Show UI box
			\nLBracket/RBracket - Move grid Left/Right\n", 13);
		...
		posTxt = new FlxText(150, 10, 0, "Grid Position: 0", 16);
	...
	function addSongUI():Void
	{
		...
		var stepperMania:FlxUINumericStepper = new FlxUINumericStepper(100, 70, 1, 0, 0, 8, 1);
	...
	override function update(elapsed:Float)
	{
		...
		rightIcon.x = gWidth / 2 + GRID_SIZE * 2 + xPosition;
		...
		if (FlxG.mouse.justPressed)
		{
			...
			{
				if (FlxG.mouse.x > xPosition
					&& FlxG.mouse.x < xPosition + gridBG.width
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.F1 && boxShow)
			{
				add(UI_box);
				boxShow = false; 
			}
			else if (FlxG.keys.justPressed.F1 && !boxShow)
			{
				remove(UI_box);
				boxShow = true; 
			}
	...
	function updatePos() {
		posTxt.text = 'Grid Position: ' + xPosition;
	}
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		note.updateHitbox();
		note.x = Math.floor(daNoteInfo * GRID_SIZE) + GRID_SIZE + xPosition;
	...
	private function addNote():Void
	{
		...
		var noteData = Math.floor((FlxG.mouse.x - GRID_SIZE - xPosition) / GRID_SIZE);
		var noteSus = 0;
		var daAlt = false;
