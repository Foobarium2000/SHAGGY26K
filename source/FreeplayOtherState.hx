...
class FreeplayOtherState extends MusicBeatState
{
	...
	var songs:Array<OtherSongMetadata> = [];
	...
	{
		songs.push(new OtherSongMetadata(songName, weekNum, songCharacter, keyCount));
	}
...
class OtherSongMetadata
{
	...
	public function new(song:String, week:Int, songCharacter:String, keyCount:String)
	{
		...
		if(week < FreeplayOtherState.coolColors.length) {
			this.color = FreeplayOtherState.coolColors[week];
		}
	}
}
