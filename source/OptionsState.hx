...
{
	var options:Array<String> = ['Language', 'Controls', 'Preferences', 'Reset data', '26K Controls'];
	...
	override function update(elapsed:Float) {
		...
		lanText.alpha = 0;
		if (curSelected == 0) lanText.alpha = 1;
...
class ControlsSubstate extends MusicBeatSubstate {
	...
	var optionShit:Array<String> = [
		...
		ClientPrefs.keyBinds[31][1],
		ClientPrefs.keyBinds[32][1],
		ClientPrefs.keyBinds[33][1],
		ClientPrefs.keyBinds[34][1],
		ClientPrefs.keyBinds[35][1],
		ClientPrefs.keyBinds[36][1],
		ClientPrefs.keyBinds[37][1],
...
class KeyboardSubstate extends MusicBeatSubstate
{
	...
	private var grpInputs:Array<AttachedText> = [];
	public static var menuBG:FlxSprite;
	public static var keyboard:FlxSprite;

	var lanText:FlxText;
	var resetScreen:FlxSprite;
	var resetText:FlxText;
    
    var keyBinds = ClientPrefs.keyBinds;
	...
	override function create() {
		#if desktop
		DiscordClient.changePresence("Selecting a keyboard type", null);
		#end
		...
		keyboard = new FlxSprite().loadGraphic(Paths.image('keyboardicon', 'shared'));
		keyboard.scale.y = 4;
		keyboard.scale.x = 4;
		keyboard.alpha = 0.34;
		keyboard.screenCenter();
		add(keyboard);
        ...
        add(alpha);
        var keyboardoptiontext:Alphabet = new Alphabet(0, 0, "SELECT YOUR KEYBOARD TYPE", true, false);
	...
	override function update(elapsed:Float) {
		...
		if (controls.BACK) {
			...
			alpha.forEachAlive(function(spr:Alphabet) {
				spr.alpha = 0;
			});
				keyboard.alpha = 0;
			close();
		...
		if (controls.ACCEPT && nextAccept <= 0) {
			switch(options[curSelected]) {
				case 'QWERTY':
                    ClientPrefs.bindQWERTY = true;
                    ClientPrefs.bindDVORAK = false;
                    ClientPrefs.bindAZERTY = false;
					ClientPrefs.bindCOLEMAK = false;
					...
					close();   
				case 'DVORAK':
                    ClientPrefs.bindQWERTY = false;
                    ClientPrefs.bindDVORAK = true;
                    ClientPrefs.bindAZERTY = false;
					ClientPrefs.bindCOLEMAK = false;
					FlxG.sound.play(Paths.sound('confirmMenu'));
					ClientPrefs.saveSettings();
